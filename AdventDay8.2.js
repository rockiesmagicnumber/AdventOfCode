...
let processData = (inputSignals) => {
    let processedData = [];
    for (let s = 0; s < inputSignals.length; s++) {
        let signal = inputSignals[s];
        signalDigits.push({ index: c });
        const signalMapping = [];
...
let processSignal = signal => {
    ...
    for (let cmd of inputs) {
        ...
        let cmdNum = getNumberByLength(cmd);
        if (cmdNum > -1) {
            signal.inputTranslated.push(cmdNum);
        } else {
            signal.inputTranslated.push(getNumberByPain(cmd));
        }        
    }
    for (let out of outputs) {
        ...
        let outNum = getNumberByLength(out);
        if (outNum > -1) {
            signal.outputTranslated.push(outNum);
        } else {
            signal.outputTranslated.push(getNumberByPain(out));
        }
    }
    return signal;
}

let getNumberByPain = cmd => {
